...
const enrichModelData = (modelId, modelName) => {
  ...
  if (id.includes('llama-3.3-70b') || id.includes('llama3-70b')) {
    return { group: 'Recommended', title: 'Smart', description: 'Balanced, high-quality responses for everyday use.', badge: 'Best', Icon: Sparkles };
  }
  if (id.includes('llama-3.1-8b') || id.includes('llama3-8b')) {
    return { group: 'Fast', title: 'Fast', description: 'Quick responses for lighter tasks.', badge: null, Icon: Zap };
  }
  if (id.includes('deepseek') || id.includes('120b') || id.includes('reasoning')) {
    return { group: 'Reasoning', title: 'Deep Thinker', description: 'Stronger reasoning for coding and complex prompts.', badge: null, Icon: Brain };
  }
  if (id.includes('allam') || id.includes('arabic')) {
    return { group: 'Multilingual', title: 'Global', description: 'Optimized for Arabic and multilingual prompts.', badge: 'Arabic', Icon: Globe };
  }
  if (id.includes('qwen') || id.includes('mixtral')) {
    return { group: 'Multilingual', title: 'Global', description: 'Strong multilingual support across general tasks.', badge: null, Icon: Globe };
  }
  if (id.includes('prompt-guard') || id.includes('safety') || id.includes('safeguard')) {
    return { group: 'Experimental', title: 'Safety Core', description: 'Evaluates prompt security boundaries.', badge: null, Icon: ShieldAlert };
  }
  if (id.includes('compound') || id.includes('agent')) {
    return { group: 'Experimental', title: 'Agentic Core', description: 'Tool-use and autonomous routing.', badge: null, Icon: Cpu };
  }
